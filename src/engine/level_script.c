...
static void level_cmd_create_instant_warp(void) {
    ...
    if (sCurrAreaIndex != -1) {
        if (gAreas[sCurrAreaIndex].instantWarps == NULL) {
            gAreas[sCurrAreaIndex].instantWarps = main_pool_alloc(INSTANT_WARP_INDEX_STOP * sizeof(struct InstantWarp));
...
static void level_cmd_create_painting_warp_node(void) {
    ...
    if (sCurrAreaIndex != -1) {
        if (gAreas[sCurrAreaIndex].paintingWarpNodes == NULL) {
            gAreas[sCurrAreaIndex].paintingWarpNodes = main_pool_alloc(NUM_PAINTINGS * sizeof(struct WarpNode));
...
static void level_cmd_3A(void) {
    ...
    if (sCurrAreaIndex != -1) {
        if ((val4 = gAreas[sCurrAreaIndex].unused) == NULL) {
            val4 = gAreas[sCurrAreaIndex].unused = main_pool_alloc(sizeof(struct UnusedArea28));
        }
